...
const MetaTags: React.FC<MetaTagsProps> = ({ page, data }) => {
  ...
  useEffect(() => {
    ...
    const setMetaTag = (attr: 'name' | 'property', value: string, content: string) => {
        if (!content) return; // Do not add empty tags
    ...
    const canonicalUrl = window.location.origin + window.location.pathname + location.hash.split('?')[0];
    ...
    let imageUrl = "https://i.hizliresim.com/cbkglw1.jpg"; // Default image
    ...
    setMetaTag('property', 'og:site_name', t('appName'));
    setMetaTag('property', 'og:locale', language);
    
    // Twitter Card
    setMetaTag('name', 'twitter:card', 'summary_large_image');
    setMetaTag('name', 'twitter:site', '@besyolmesam');
